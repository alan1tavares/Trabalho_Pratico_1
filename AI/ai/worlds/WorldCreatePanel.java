...
public class WorldCreatePanel extends JPanel implements ActionListener, ItemListener {
	...
	public WorldCreatePanel(JFrame f) {
		...
		northNorthPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createRaisedBevelBorder(),
				BorderFactory.createLoweredBevelBorder()));
		northPanel.add(northNorthPanel, BorderLayout.NORTH);

		northCenterPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createRaisedBevelBorder(),
				BorderFactory.createLoweredBevelBorder()));
		...
		southPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createRaisedBevelBorder(),
				BorderFactory.createLoweredBevelBorder()));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (action.equals("Build Random"))
			build();
		else if (action.equals("Run Trials"))
			trials();
		else if (action.equals("Load Wumpus World"))
			load();
		else if (action.equals("Rebuild World"))
			rebuild();
		else if (action.equals("Create a Wumpus World"))
			createWorld();
		else if (action.equals("save"))
			wwe.save();
		else if (action.equals("Simulate an Agent in an Environment"))
			simulate();
	}
	...
	private void createWorld() {
		...
		centerPanel.add(
				new JLabel(
						"Directions: Right-click in a cell you'd like to add something to.  Check what you'd like to add.  When you are satisfied with your world, save it."),
				BorderLayout.NORTH);
		add(centerPanel);
		centerPanel.setVisible(false);
		centerPanel.setVisible(true);
	...
	private Agent createAgent(String agentName) {
		if (agentName == "Random Vacuum Agent")
			return new RandomVacuumAgent();
		else if (agentName == "Reactive Vacuum Agent")
			return new ReactiveVacuumAgent();
		else if (agentName == "Random Wumpus Agent")
			return new RandomWumpusAgent();
		else if (agentName == "Logic Testing Agent")
		...
		public void run() {
			...
			for (int j = 0; j < Integer.parseInt(numTrials.getText()); j++) {
				...
				for (int i = 0; i < agts.length; i++) {
					...
					if ((String) envs.getSelectedItem() == "Wumpus World")
						((WumpusWorld) w).w = ((WumpusWorld) world).w;
